...
export class TodosService {
    ...
    async getFilteredTodos(filterDto: FilterTodosDto) : Promise<Todo[]> {
        ...
        if (!!filterDto.isCompleted) {
            allTodos = allTodos.filter(t => t.isCompleted === filterDto.isCompleted);
        }
        if (filterDto.search) {
            allTodos = allTodos.filter(t => t.taskName.indexOf(filterDto.search) > -1);
        }
        return allTodos;
    }
    ...
    async patchTodo(id: string, dto: PatchTodoDto): Promise<Todo> {
        ...
        todo = {...todo, ...{taskName: dto.taskName, isCompleted: dto.isCompleted}};
        await this.todoModel.updateOne({"TodoId": id}, {$set: {...mapFromDto(todo)}}).exec();
        return todo;
    }
